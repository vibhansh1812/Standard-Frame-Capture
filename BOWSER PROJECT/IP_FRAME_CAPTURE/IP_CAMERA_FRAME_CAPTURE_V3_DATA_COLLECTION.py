...
FRONT_CAM_IP='192.168.1.202'
TOP_CAM_IP='192.168.1.201'
...
BASE_SAVE_PATH = "/home/insightzz/Insightzz/Code/ALGORITHM/IP_IMG/"
...
def cam_ip_101_save():
    ...
    while not(module_stop):
        ...
        try:
            if ret:
                cv2.imwrite(f"{BASE_SAVE_PATH}CAM_202/TMP.jpg",frame)
                START_FRAMECAPTURE_DATETIME = datetime.datetime.now()
                if os.listdir(f"{BASE_SAVE_PATH}CAM_202/TMP/") == []:
                    shutil.copyfile(f"{BASE_SAVE_PATH}CAM_202/TMP.jpg",f"{BASE_SAVE_PATH}CAM_202/TMP/TMP.jpg")
            else:
...
def cam_ip_102_save():
    ...
    while not(module_stop):
        ...
        try:
            if ret:
                cv2.imwrite(f"{BASE_SAVE_PATH}CAM_201/TMP.jpg",frame)
                START_FRAMECAPTURE_DATETIME = datetime.datetime.now()
                if os.listdir(f"{BASE_SAVE_PATH}CAM_201/TMP/") == []:
                    shutil.copyfile(f"{BASE_SAVE_PATH}CAM_201/TMP.jpg",f"{BASE_SAVE_PATH}CAM_201/TMP/TMP.jpg")
            else:
                timeDiff = datetime.datetime.now() - START_FRAMECAPTURE_DATETIME
                if timeDiff > fiveSecDiff:
                    START_FRAMECAPTURE_DATETIME = datetime.datetime.now()
